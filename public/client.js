...
function renderPlayers(players){
  ...
  players.forEach(p=>{
    ...
    // 自分の欄だけ実際に打った単語の履歴を表示する。他の参加者の欄には
    // 単語の中身は表示せず、正解した回数だけを表示する(ネタバレ防止)。
    const wordsList = col.querySelector('.p-words');
    if (p.id === myId && Array.isArray(p.words)) {
      if (wordsList.dataset.count != p.words.length || wordsList.dataset.mode !== 'words') {
        wordsList.innerHTML = p.words.map(w => `<li>${escapeHtml(w)}</li>`).join('');
        wordsList.dataset.count = p.words.length;
        wordsList.dataset.mode = 'words';
      }
    } else {
      if (wordsList.dataset.count != p.wordCount || wordsList.dataset.mode !== 'count') {
        wordsList.innerHTML = `<li class="word-count-only">正解 ${p.wordCount}回</li>`;
        wordsList.dataset.count = p.wordCount;
        wordsList.dataset.mode = 'count';
      }
    }
...
socket.on('gameEnd', ({ players }) => {
  ...
  players
    ...
    .forEach(p=>{
      ...
      row.innerHTML = `<span>${escapeHtml(p.name)}${p.eliminated ? '(脱落)' : '(生存)'}</span><span>${p.wordCount}回</span>`;
