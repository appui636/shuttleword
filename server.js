...
// viewerId を指定すると、そのプレイヤー自身の分だけ実際に打った単語(words)を含める。
// 他の参加者には正解数(wordCount)のみを返し、単語の中身は一切含めない(ネタバレ防止)。
...
// room内の全員に、それぞれの視点向けにpersonalizeされたplayers配列を含めて個別配信する
function broadcastPersonalized(room, event, extra) {
  for (const id of room.players.keys()) {
    io.to(id).emit(event, { ...extra, players: publicPlayers(room, id) });
  }
}
...
function endGame(room) {
  ...
  broadcastPersonalized(room, 'gameEnd', {
    survivors: survivors.map(p => ({ id: p.id, name: p.name, wordCount: p.words.length }))
  });
}
...
function startGameLoop(room) {
  ...
  room.timer = setInterval(() => {
    ...
    broadcastPersonalized(room, 'tick', {
      elapsed,
      lap: entry.lap - 1,
      level: entry.level,
...
io.on('connection', socket => {
  socket.on('createRoom', ({ name, themeId }) => {
    ...
    broadcastPersonalized(room, 'roomUpdate', { hostId: room.hostId, theme: theme.name });
  });
  ...
  socket.on('joinRoom', ({ code, name }) => {
    ...
    broadcastPersonalized(room, 'roomUpdate', { hostId: room.hostId, theme: room.theme.name });
  });
  ...
  socket.on('submitWord', ({ word }) => {
    ...
    // 実際に打った単語(word)は本人にだけ通知する。他の参加者には正解/不正解の結果のみ通知し、
    // 単語の中身は一切送らない(何を打ったかネタバレしないようにするため)。
    for (const id of room.players.keys()) {
      const payload = {
        playerId: socket.id,
        ...
        correct,
        reason,
        players: publicPlayers(room, id),
        totalWords: totalWordsCount(room)
      };
      if (id === socket.id) payload.word = trimmed;
      io.to(id).emit('wordResult', payload);
    }
  });
